...
* *Preview website* is available only when `Build/` holds a site generated for localhost. A site generated for a published host puts every link under `/hub/`, which the preview server doesn't have, so its pages would load unstyled and every link would fail. The app checks the site on disk (`sitemap.xml`) rather than Settings, so a localhost build stays previewable after you switch the target or relaunch.
...
* Built using v3.4.0 of the Photo Club Hub Data package.
